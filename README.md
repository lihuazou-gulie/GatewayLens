...
[MIT](LICENSE) · [贡献指南](CONTRIBUTING.md) · [安全说明](SECURITY.md)

独立部署的 Sub2API 监控面板。网页输入站点地址和全局管理员 API Key，选择需要监控的分组，即可使用总览、分组和模型专题三个视图。提供默认深色和星谷像素两套主题，以及仪表盘、容量条、趋势图和号池警报。

界面使用中文，当前适合单实例自托管。以下截图来自本地模拟数据。


<details>
<summary>星谷像素主题</summary>
...
自动构建并启动服务，更新已有实例时自动备份。宿主机无需安装 Node.js。默认监听 `127.0.0.1:8787`；支持已有 HTTPS 代理，也支持一个容器直接提供 HTTPS。
...
| 设置     | `/settings` | 初始化、登录、连接、分组、模块、主动探测和密码管理       |
...
浏览器测试自动完成初始化、连接和展示设置、模拟探测、双主题桌面/手机页面及权限检查，输出截图后退出。开发与验证方式见上方命令。
