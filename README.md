...
各页面页脚显示当前运行版本，并提供 GitHub 项目与更新日志入口。版本取自随程序部署的 `package.json`；发布记录在 GitHub 查看，页面不会后台查询新版本或自动升级。
...
更新前查看 GitHub 的 Release 说明，确认目标版本是否有配置或数据迁移要求。在原部署目录操作，并沿用原来的 `.env`、Compose 项目名称与数据卷。

通过 Git 克隆并跟随 `main` 的实例：
...
先确认 `git status` 没有需要保留的源码修改；若拉取失败或存在本地改动，处理后再部署，不要用强制重置覆盖自己的文件。只执行 `git pull` 不会更新正在运行的容器，必须继续执行部署脚本。

脚本会先构建镜像并运行测试，构建失败时现有服务继续运行。构建成功后备份现有配置卷和旧镜像，再替换面板容器并等待健康检查；启动不健康时尝试恢复旧镜像。容器替换会短暂中断访问，并使已有登录会话失效。正常更新保留站点文案、连接凭证、展示分组、探测设置和历史数据，不需要重新初始化。

部署固定 Release 的用户先运行 `git fetch --tags`，再用 `git switch --detach <目标版本标签>` 切换到已发布的标签，最后运行部署脚本；尖括号内容需替换为 Release 页上的真实标签。

下载 ZIP 的用户先备份原源码目录（包含自己的 `.env`），把新版本完整解压到单独的源码目录，复制原 `.env` 并确认 Compose 项目名称、数据卷和证书挂载与原实例一致，再运行部署脚本。默认项目名称固定为 `sub2api-monitor`，数据位于命名卷中；自定义过名称的实例必须保留自己的名称。不要直接覆盖解压而留下新版本已删除的旧文件。

更新后检查页脚版本并重新登录。手动回滚可使用部署脚本输出的备份目录，命令见下方。若 Release 说明有不可逆的数据迁移，按该版本的备份恢复要求操作；恢复旧镜像本身不会撤销数据变化。
