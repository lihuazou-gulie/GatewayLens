...
async function refresh() {
  ...
  try {
    const data = await api.request("monitor", {
      query: { range: state.range, scope: state.scope, topic: state.topic, model: state.model },
    });
    if (id !== sequence) return;
    const names = state.model
      ? state.knownModels
      : [...new Set((data.models?.data?.rows || []).map((m) => m.model))];
...
const poller = new Poller({
  intervalMs: 30000,
  tick: () => (document.visibilityState === "hidden" ? Promise.resolve() : refresh()),
});
...
document.querySelectorAll("[data-range]").forEach((button) =>
  button.addEventListener("click", () => {
    store.set({ range: button.dataset.range });
    void refresh();
  }),
);
for (const [id, key] of [
  ...
  document.getElementById(id).addEventListener("change", (event) => {
    store.set({
      ...
      ...(key === "topic" ? { model: "", knownModels: [] } : {}),
    });
    void refresh();
  });
}
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") void poller.trigger();
});
try {
