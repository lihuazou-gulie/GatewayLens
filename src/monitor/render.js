import { setText, show, element, options, writeText, setVisible, setAttribute, toggleClass } from "../ui/dom.js";
...
import { createMetricCard, createGroupCard, createModelCard, createStatRow, createNote, statuses } from "./cards.js";
...
export function createMonitorRenderer() {
  ...
  const models = list("model-grid", byKey, (item) => item.model ? createModelCard() : createNote("empty-panel"));
  const pool = list("pool-content", byKey, (item) => item.key === "note" ? createNote() : createStatRow());
  const system = list("system-grid", byKey, (item) => item.key === "note" ? createNote() : createMetricCard());
  const chart = document.getElementById("trend-chart");
  const notice = document.getElementById("notice"), noticeText = element("span"), settingsLink = element("a", "inline-link", "前往面板设置 →");
  settingsLink.href = "/settings"; notice.append(noticeText, settingsLink);

  return function render(state) {
    const { data, error, view, loading } = state; const modelsView = view === "models";
    document.querySelectorAll("[data-view]").forEach((node) => setAttribute(node, "aria-current", node.dataset.view === view ? "page" : "false"));
    document.querySelectorAll("[data-range]").forEach((node) => setAttribute(node, "aria-pressed", node.dataset.range === state.range));
    ...
    setVisible(notice, Boolean(error) || data?.state === "unconfigured" || data?.state === "partial");
    writeText(noticeText, error ? `${error}${data ? " · 下方保留上次成功快照" : ""}` : data?.state === "unconfigured" ? "还没有配置监控分组，请到面板设置完成接入。" : data?.state === "partial" ? "部分指标不可用，各模块已分别标记状态。" : "");
    ...
    setAttribute(document.getElementById("status-dot"), "data-state", error ? "error" : loading ? "loading" : data?.state || "");
    const refresh = document.getElementById("refresh-button"); if (refresh.disabled !== loading) refresh.disabled = loading;
    ...
    const topic = document.getElementById("topic-select"); if (topic.value !== state.topic) topic.value = state.topic;
    const items = summaryMetrics(data, modelsView), p = data.summary.pool;
    show("metrics", (!modelsView || state.topic === "images") && items.length > 0); metrics.update(items);
    const critical = !error && p?.percent !== null && p?.percent !== undefined && !p.partial && p.percent < data.poolThreshold;
    show("pool-health-alert", critical); toggleClass(document.body, "pool-health-critical", critical);
    ...
    drawTrendChart(chart, data.traffic.map((p) => ({ started_at: p.at, total: p.requests, success_rate: null })), state.range);
    setText("trend-note", data.traffic.length ? "请求量为上游统计口径，包含业务限制；与上方服务请求口径不同。" : "暂无趋势数据，或上游未提供趋势接口。");
    pool.update(poolRows(p));
    setText("group-count", `${data.groups.length} 个分组`); groups.update(data.groups);
    const rows = data.models?.data?.rows || [], image = state.topic === "images";
    ...
    models.update(rows.length ? rows.map((model) => ({ key: JSON.stringify([model.platform, model.model]), model, image })) : [{ key: "empty", text: data.models.state === "ok" ? "所选范围暂无模型请求样本" : "等待可用的模型统计" }]);
