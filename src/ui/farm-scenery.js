...
const scenes = [
  { host: ".topbar, .settings-topbar", name: "header", props: [
    ["tree", "tree"], ["tree", "tree-far"], ["fence", "fence"],
    ["barrel", "barrel"], ["flowers", "flowers"], ["berries", "berries"],
    ["chicken", "chicken"],
  ] },
  { host: "#overview-panels", name: "meadow", props: [
    ["fence", "fence"], ["flowers", "flowers"], ["berries", "berries"],
    ["grass", "grass"], ["rocks", "rocks"], ["mushrooms", "mushrooms"],
    ["stump", "stump"], ["sign", "sign"],
    ["cow", "cow"], ["sheep", "sheep"],
  ] },
  { host: "footer", name: "footer", props: [
    ["tree", "tree"], ["cave", "cave"], ["mushrooms", "mushrooms"],
    ["pond", "pond"], ["bridge", "bridge"], ["flowers", "flowers"],
    ["stump", "stump"], ["fence", "fence"], ["crate", "crate"],
    ["barrel", "barrel"], ["coins", "coins"], ["campfire", "campfire"], ["berries", "berries"],
    ["duck", "duck"], ["cat", "cat"], ["dog", "dog"],
  ] },
];

function sprite(key, slot, eager) {
  const art = farmArtwork[key], [left, top, width, height] = art.bounds;
  ...
  image.src = art.src; image.alt = ""; image.width = art.size[0]; image.height = art.size[1];
  image.loading = eager ? "eager" : "lazy"; image.decoding = "async"; image.draggable = false;
  node.append(image);
  return node;
}
...
function terrain(name) {
  ...
  image.setAttribute("width", String(art.size[0])); image.setAttribute("height", String(art.size[1]));
  pattern.append(image); defs.append(pattern);
...
export function initFarmScenery() {
  ...
  const mount = () => {
    ...
    for (const scene of scenes) {
      ...
      node.append(...scene.props.map(([key, slot]) => sprite(key, slot, scene.name === "header")), terrain(scene.name));
