...
import {
  ...
  DEMO_PROBE_KEY,
  DEMO_GROUPS,
} from "./fixtures/sub2api.mjs";
import { qualitySummary, capacityUnion } from "../server/modules/monitoring/domain/metrics.mjs";
...
async function harness(t, fixtureOptions = {}) {
  ...
  const source = fixtureSub2Api(fixtureOptions);
  const upstreamPort = await listen(source);
  const app = await createGatewayLensServer({
    config: { ...loadConfig({ KANBAN_DATA_DIR: dir }), cacheMs: 10 },
  });
  ...
  async function request(
    path,
    { body, method = body ? "POST" : "GET", auth = true, originHeader = origin } = {},
  ) {
    const res = await fetch(`${origin}/api/${path}`, {
      method,
      headers: {
        ...(auth && cookie ? { Cookie: cookie } : {}),
        ...(body ? { "Content-Type": "application/json", Origin: originHeader } : {}),
      },
  ...
  async function setup() {
    return request("setup", { body: { code: app.identity.setupCode, password: PASSWORD } });
  }
  async function connect() {
    return request("admin/connection", {
      body: {
...
test("starts without Runtime or Sub2API and protects one-time initialization", async (t) => {
  ...
  assert.equal(
    (await h.request("setup", { body: { code: "wrong", password: PASSWORD } })).status,
    401,
  );
  assert.equal(
    (
      await h.request("setup", {
        body: { code: h.identity.setupCode, password: PASSWORD },
  ...
  assert.match(setup.res.headers.get("set-cookie"), /HttpOnly; SameSite=Strict/);
...
test("setup race has exactly one winner", async (t) => {
  const h = await harness(t);
  const result = await Promise.all([h.setup(), h.setup()]);
...
test("connection encrypts credentials and survives process restart", async (t) => {
  const h = await harness(t);
  await h.setup();
  assert.equal((await h.connect()).status, 200);
  const persisted = await readFile(join(h.dir, "settings.json"), "utf8");
  assert.ok(!persisted.includes(DEMO_KEY));
  assert.ok(!persisted.includes(PASSWORD));
  const restarted = await openPersistence(h.dir);
  assert.equal(
    new ConfigurationRepository(restarted.store, restarted.vault).snapshot().connection.apiKey,
    DEMO_KEY,
  );
  const admin = await h.request("admin/settings");
  assert.ok(!JSON.stringify(admin.data).includes(DEMO_KEY));
  assert.deepEqual((await readdir(h.dir)).sort(), [
    "encryption.key",
    "settings.json",
    "setup-code",
  ]);
...
test("public metrics whitelist fields, deduplicate shared accounts, and bound selected scopes", async (t) => {
  ...
  await h.connect();
  await h.display();
  const result = await h.request("monitor", { auth: false });
  ...
  assert.equal(result.data.summary.capacity.max, 40);
  assert.equal(result.data.summary.pool.total, 5);
  ...
  const group = await h.request(`monitor?scope=${result.data.groups[0].scope}`, { auth: false });
  assert.equal(group.data.groups.length, 1);
...
test("private dashboard and hidden modules are enforced on backend", async (t) => {
  ...
  await h.display({
    modules: { quality: true, traffic: false, capacity: false, pool: false, models: false },
  });
  const data = (await h.request("monitor", { auth: false })).data;
  assert.equal(data.summary.capacity, null);
  assert.equal(data.summary.pool, null);
  assert.equal(data.models.data, null);
...
test("guests can view configured groups by default while setup and administration remain private", async (t) => {
  ...
  assert.equal(
    (
      await h.request("admin/display", {
        method: "PUT",
        body: { revision: settings.revision, display },
      })
    ).status,
    200,
  );
  ...
  assert.equal(guest.data.groups.length, 1);
  assert.equal(guest.data.groups[0].label, "公开分组");
  assert.equal("system" in guest.data, false);
  ...
  assert.equal(
    (
      await h.request("admin/display", {
        auth: false,
        method: "PUT",
        body: { revision: h.persistence.store.revision, display },
      })
    ).status,
    401,
  );
  assert.equal((await openPersistence(h.dir)).store.read().display.public, true);
  await h.request("login", { body: { password: PASSWORD } });
  await h.display({ public: false });
  assert.equal((await h.request("monitor", { auth: false })).status, 401);
  assert.equal((await openPersistence(h.dir)).store.read().display.public, false);
});
test("partial endpoints keep healthy sections and do not fabricate zero metrics", async (t) => {
  const failures = new Map([
    ["/admin/ops/concurrency", 403],
    ["/admin/channel-monitor-v2/models", 404],
  ]);
  const h = await harness(t, { failures });
  await h.setup();
  await h.connect();
  await h.display();
  ...
  assert.equal(data.summary.capacity, null);
  assert.equal(data.groups[0].capacity.state, "denied");
  assert.equal(data.models.state, "unsupported");
...
test("image topic only returns exact configured model names", async (t) => {
  ...
  assert.deepEqual(
    data.models.data.rows.map((r) => r.model),
    ["gpt-image-2"],
  );
  assert.equal(data.models.data.rows[0].rate, 98);
  assert.equal(data.models.data.rows[0].cacheRate, 36.4);
  assert.equal(
    (await h.request("monitor?topic=images&model=unlisted", { auth: false })).status,
    400,
  );
});
...
test("malformed realtime data affects only its group and section", async (t) => {
  ...
  assert.equal(data.summary.capacity.partial, true);
  assert.equal(data.summary.capacity.max, 32);
  assert.equal(data.summary.pool.partial, true);
  assert.equal(data.summary.pool.total, 4);
  assert.ok(data.summary.quality.requests > 0);
});
...
test("a settings change discards an in-flight snapshot from its old source", async (t) => {
  ...
  await h.display();
  let finish;
  const original = h.monitor.load.bind(h.monitor);
  h.monitor.load = async (...args) => {
    await new Promise((r) => {
      finish = r;
    });
  ...
  await h.configuration.saveDisplay({
    revision: h.persistence.store.revision,
    display: { ...h.configuration.settings().display, public: false, groups: [] },
  });
  finish();
  await assert.rejects(pending, (e) => e.code === "conflict");
  await assert.rejects(h.monitor.snapshot({}, false), (e) => e.code === "unauthenticated");
});
test("settings changes detect stale revisions and source changes reset published scopes", async (t) => {
  ...
  assert.equal(
    (
      await h.request("admin/display", {
        method: "PUT",
        body: { revision: 0, display: h.configuration.settings().display },
      })
    ).status,
    409,
  );
  ...
  const result = await h.request("admin/connection", {
    body: {
  ...
  assert.equal(result.status, 200);
  assert.deepEqual(h.configuration.settings().display.groups, []);
  assert.equal(h.configuration.settings().display.public, true);
  assert.equal((await h.request("monitor", { auth: false })).status, 401);
});
test("static paths deny settings files, source internals, and old Runtime APIs", async (t) => {
  const h = await harness(t);
  for (const path of ["/", "/groups", "/models", "/settings"])
    assert.equal((await fetch(h.origin + path)).status, 200);
  ...
  for (const path of [
    ...
    assert.equal((await fetch(h.origin + path)).status, 404);
});
test("logout invalidates the session and password updates require the current password", async (t) => {
  ...
  assert.equal(
    (
      await h.request("admin/password", {
        body: { currentPassword: "wrong", password: PASSWORD + "x" },
      })
    ).status,
    401,
  );
...
test("cache shares in-flight collection beyond TTL and removes rejected entries", async () => {
  const cache = new TimedValueCache(1);
  let finish;
  let calls = 0;
  const a = cache.get("one", () => {
    calls++;
    return new Promise((resolve) => (finish = resolve));
  });
  await new Promise((r) => setTimeout(r, 10));
  const b = cache.get("one", () => ++calls);
  finish(42);
  ...
  assert.equal(await cache.get("error", () => 3), 3);
});
test("URL validation accepts sites and API bases and rejects credential-bearing URLs", () => {
  ...
  for (const site of [
    ...
    assert.throws(() => normalizeSite(site));
...
test("configured probes run through the gateway and remain admin-only", async (t) => {
  ...
  assert.equal(
    (
      await h.request("admin/probe", {
        method: "PUT",
        body: { revision: settings.revision, probe, apiKey: DEMO_PROBE_KEY },
      })
    ).status,
    200,
  );
  ...
  assert.equal(configured.config.groupLabel, "标准文本");
  ...
  assert.equal(run.data.reason, "ok");
  const status = (await h.request("admin/probe")).data;
  assert.equal(status.latest.status, "ok");
  assert.equal(status.latest.latencyMs >= 0, true);
  assert.equal((await h.request("admin/probe", { auth: false })).status, 401);
  const persisted = await readFile(join(h.dir, "settings.json"), "utf8");
  assert.ok(!persisted.includes(DEMO_PROBE_KEY));
});
...
test("changing the source or displayed groups invalidates the active probe target", async (t) => {
  ...
  assert.equal(
    (
      await h.request("admin/display", {
        method: "PUT",
        body: { revision: settings.revision, display: reduced },
      })
    ).status,
    200,
  );
  assert.equal(h.repositories.probes.read().config.enabled, false);
  assert.equal(h.repositories.probes.read().apiKey, "");
  await h.display();
  settings = (await h.request("admin/settings")).data;
  const source = fixtureSub2Api();
  const port = await listen(source);
  ...
  assert.equal(
    (
      await h.request("admin/connection", {
        body: { site: `http://127.0.0.1:${port}`, apiKey: DEMO_KEY, revision: settings.revision },
      })
    ).status,
    200,
  );
