...
import { ProbeClient } from "../server/modules/probing/infrastructure/openai-probe-client.mjs";
import { ProbeResultStore } from "../server/modules/probing/infrastructure/probe-result-repository.mjs";
import { ProbeService } from "../server/modules/probing/application/probe-service.mjs";
import { validateProbe } from "../server/modules/probing/domain/policy.mjs";
...
test("probe client sends a bounded OpenAI-compatible request and stores no response text", async () => {
  ...
  const client = new ProbeClient({
    baseUrl: "https://example.test/panel/api/v1",
    apiKey: "probe-secret",
  ...
  assert.match(request.url, /^https:\/\/example\.test\/panel\/v1\/chat\/completions$/);
  assert.equal(request.options.method, "POST");
  assert.equal(request.options.redirect, "error");
  assert.equal(request.options.headers.Authorization, "Bearer probe-secret");
  assert.equal(request.options.headers["X-API-Key"], "probe-secret");
...
test("probe client distinguishes HTTP, invalid and empty responses", async () => {
  ...
  const cases = [
    ...
    [response(JSON.stringify({ choices: [{ message: { content: " " } }] })), "empty_response"],
  ];
...
test("probe results persist atomically and can be scoped to a configuration generation", async () => {
  ...
  const store = new ProbeResultStore(dir);
  ...
  const values = await store.list("gen-a");
  assert.equal(values.length, 1);
  assert.equal(values[0].status, "ok");
  assert.ok(
    !JSON.parse(await readFile(join(dir, "probe-results.json"), "utf8")).some(
      (item) => item.apiKey,
    ),
  );
});
...
test("probe service executes a configured target and exposes sanitized history", async () => {
  ...
  const resultStore = {
    append: async (value) => records.push(value),
    list: async () => records.slice().reverse(),
  };
  const service = new ProbeService({
    repository: { read: () => store.probe() },
    configurations: {
      snapshot: () => ({ connection: store.connection(), display: store.state.display }),
    },
    timeoutMs: 1000,
    resultStore,
    now: () => now,
    schedulerMs: 60000,
    clientFactory: () => ({
      probe: async () => ({ status: "ok", reason: "ok", latencyMs: 11, upstreamStatus: 200 }),
    }),
  });
  const result = await service.runNow();
  assert.equal(result.status, "ok");
  assert.equal(records[0].groupId, 7);
  assert.equal(records[0].model, "gpt-5.6");
  const status = await service.status(10);
  assert.equal(status.config.groupLabel, "文本组");
  assert.equal(status.latest.latencyMs, 11);
  assert.ok(!JSON.stringify(status).includes("probe"));
  now += 300000;
});
...
test("probe scheduler waits for the configured interval and reruns the target", async () => {
  ...
  const store = {
    dir: ".",
    revision: 1,
    state: { display: { groups: [{ id: 7, label: "文本组" }] } },
    connection: () => ({ baseUrl: "https://example.test/api/v1", apiKey: "admin" }),
    probe: () => ({
      generation: "g1",
      apiKey: "probe",
  ...
  const service = new ProbeService({
    ...
    timeoutMs: 1000,
    resultStore: { append: async () => {}, list: async () => [] },
    now: () => now,
    clientFactory: () => ({
      probe: async () => {
        calls += 1;
        return { status: "ok", reason: "ok", latencyMs: 1, upstreamStatus: 200 };
      },
    }),
  });
  await service.tick();
...
test("settings version 1 is upgraded with a disabled probe configuration", async () => {
  const dir = await mkdtemp(join(tmpdir(), "probe-migration-"));
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      admin: null,
      connection: null,
      display: first.store.read().display,
    }),
  );
  const upgraded = await openPersistence(dir);
  assert.equal(upgraded.store.read().version, 2);
  assert.equal(upgraded.store.read().probe.config.enabled, false);
  assert.equal(upgraded.store.read().probe.secret, null);
});
