...
async function request(
  path,
  { body, method = body ? "POST" : "GET", auth = true, status = 200 } = {},
) {
...
assert.equal(publicData.summary.capacity.max, 40);
assert.equal(
  /SENSITIVE|account_id|user_email|baseUrl|secret|system/.test(JSON.stringify(publicData)),
  false,
);
const images = await request("monitor?topic=images&range=30d", { auth: false });
assert.deepEqual(
  images.models.data.rows.map((m) => m.model),
  ["gpt-image-2"],
);
...
assert.equal(stored.includes(DEMO_KEY), false);
assert.equal(stored.includes(password), false);
await request("logout", { body: {} });
console.log(
  JSON.stringify({
    status: "passed",
    restart: boot.initialized,
    publicGroups: 3,
    uniqueAccounts: 5,
    imageRate: 98,
