...
export function captureCompose(container) {
  ...
  if (!project || config.Labels?.["com.docker.compose.service"] !== "runtime-command-center")
  ...
  Object.entries(container.NetworkSettings.Networks).forEach(([name, network], i) => {
    const key = `network_${i}`;
    networks[key] = { external: true, name };
  ...
  const service = {
    ...
    restart: host.RestartPolicy.Name || "no",
    cap_drop: host.CapDrop || [],
    cap_add: host.CapAdd || [],
    security_opt: host.SecurityOpt || [],
    tmpfs: Object.entries(host.Tmpfs || {}).map(([path, flags]) => `${path}:${flags}`),
    ports: Object.entries(host.PortBindings || {}).flatMap(([target, bindings]) =>
      (bindings || []).map((binding) => ({
        target: Number(target.split("/")[0]),
        protocol: target.split("/")[1] || "tcp",
        published: binding.HostPort,
        host_ip: binding.HostIp || "0.0.0.0",
      })),
    ),
    logging: { driver: host.LogConfig.Type, options: host.LogConfig.Config || {} },
    ...(host.NanoCpus ? { cpus: host.NanoCpus / 1e9 } : {}),
    ...(host.Memory ? { mem_limit: host.Memory } : {}),
    ...(host.PidsLimit ? { pids_limit: host.PidsLimit } : {}),
    ...(health
      ? {
          healthcheck: {
            test: health.Test,
            ...(health.Interval ? { interval: `${health.Interval}ns` } : {}),
            ...(health.Timeout ? { timeout: `${health.Timeout}ns` } : {}),
            ...(health.StartPeriod ? { start_period: `${health.StartPeriod}ns` } : {}),
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  process.stdout.write(
    JSON.stringify(captureCompose(JSON.parse(readFileSync(0, "utf8"))[0]), null, 2),
  );
}
